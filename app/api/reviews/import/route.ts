...
export async function POST(request: Request) {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      const location = await prisma.location.findFirst({ where: { id: locationId, workspaceId } });
      if (!location) return NextResponse.json({ error: 'Location not found in workspace' }, { status: 400 });
    ...
    await assertWorkspaceAccess(userId, parsed.data.workspaceId, [Role.OWNER, Role.ADMIN, Role.MEMBER]);

    const location = await prisma.location.findFirst({
      where: { id: parsed.data.locationId, workspaceId: parsed.data.workspaceId }
    });
    if (!location) return NextResponse.json({ error: 'Location not found in workspace' }, { status: 400 });
